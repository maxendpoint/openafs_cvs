...
RCSID("$Header: /cvs/openafs/src/ntp/Attic/ntpd.c,v 1.6 2002/08/21 18:13:43 shadow Exp $");
	...
	char *argv[];
{
	...
	syslog(LOG_NOTICE, "%s version $Revision: 1.6 $", prog_name);
	syslog(LOG_NOTICE, "patchlevel %d", PATCHLEVEL);

#ifdef	DEBUG
	if (debug)
		printf("%s version $Revision: 1.6 $ patchlevel %d\n",
